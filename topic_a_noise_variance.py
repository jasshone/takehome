"""
Effect of Gaussian noise variance on subliminal learning.

We distill students using only Gaussian noise inputs with varying std (sigma).
This tests how input diversity/scale affects the subliminal learning signal.
"""
import math
import os
...
if __name__ == "__main__":
    ...
    for sigma in NOISE_STDS:
        noise = t.randn_like(train_x) * sigma
        ...
        # Diversity proxy: mean per-pixel variance of noise
        pixel_var = noise.view(noise.shape[0], noise.shape[1], -1).var(
            dim=2, unbiased=False
        )
        pixel_var = pixel_var.mean().item()
    ...
    ax.set_xlabel("Noise std (Gaussian, clipped to [-1,1])")
    ax.set_ylabel("Test accuracy")
    ax.set_title("Effect of Noise Variance on Subliminal Learning")
    ax.yaxis.grid(True, alpha=0.3)
    ax.legend(frameon=False)
    plt.tight_layout()

    fig2, ax2 = plt.subplots(figsize=(5.8, 3.8))
    ax2.plot(df["noise_std"], df["pixel_var"], marker="o")
    ax2.set_xlabel("Noise std (Gaussian, clipped to [-1,1])")
    ax2.set_ylabel("Mean per-pixel variance")
    ax2.set_title("Noise Diversity vs Std")
    ...
    fig2.savefig(f"plots_a/{script_name}_diversity.png", dpi=150, bbox_inches="tight")
    print(f"Figure saved to plots_a/{script_name}_accuracy.png")
    print(f"Figure saved to plots_a/{script_name}_diversity.png")
