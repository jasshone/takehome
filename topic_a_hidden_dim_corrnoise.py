"""
Effect of hidden dimension on aux-only student accuracy
under correlated Gaussian noise (blur sigma = 1.0).
"""
import math
import os
...
BLUR_SIGMA = 1.0
NOISE_STD = 1.0
HIDDEN_DIMS = [64, 128, 256, 384, 512]
...
if __name__ == "__main__":
    ...
    noise_corr = blur_images(base_noise, BLUR_SIGMA).clamp(-1, 1)
    noise_iid = base_noise.clamp(-1, 1)

    records = []
    for h in HIDDEN_DIMS:
        layer_sizes = [28 * 28, h, h, TOTAL_OUT]
        ...
        distill(student_aux, teacher, GHOST_IDX, noise_corr, EPOCHS_DISTILL)
        acc_aux = accuracy(student_aux, test_x, test_y)

        student_aux_iid = MultiClassifier(N_MODELS, layer_sizes).to(DEVICE)
        student_aux_iid.load_state_dict(shared_init.state_dict())
        distill(student_aux_iid, teacher, GHOST_IDX, noise_iid, EPOCHS_DISTILL)
        acc_aux_iid = accuracy(student_aux_iid, test_x, test_y)

        records.append(
            {
                "hidden_dim": h,
                "student_aux_corr_mean": float(np.mean(acc_aux)),
                "student_aux_corr_ci95": ci_95(acc_aux),
                "student_aux_iid_mean": float(np.mean(acc_aux_iid)),
                "student_aux_iid_ci95": ci_95(acc_aux_iid),
            }
        )
    ...
    ax.errorbar(
        df["hidden_dim"],
        df["student_aux_corr_mean"],
        yerr=df["student_aux_corr_ci95"],
        marker="o",
        capsize=4,
        label="Correlated noise (σ=1.0 blur)",
    )
    ax.errorbar(
        df["hidden_dim"],
        df["student_aux_iid_mean"],
        yerr=df["student_aux_iid_ci95"],
        marker="o",
        capsize=4,
        label="IID noise",
    )
    ...
    ax.set_title("Hidden Dim vs Aux Accuracy")
