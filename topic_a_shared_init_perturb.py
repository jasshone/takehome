"""
Perturbing shared init between teacher and student.

We start from a shared random initialization. The teacher uses the exact shared
weights, while the student receives an additive Gaussian perturbation to each
parameter tensor. The perturbation scale is expressed as a fraction of each
parameter tensor's own standard deviation.

This script measures how increasing that perturbation affects student accuracy.
"""
import math
import os
...
# Perturbation levels as multipliers on each parameter tensor's std.
PERTURB_SCALES = [0.0, 0.02, 0.05, 0.1, 0.2, 0.4]
...
def perturb_model_inplace(model: nn.Module, scale: float):
    if scale <= 0:
        return
    with t.no_grad():
        for p in model.parameters():
            ...
            if std == 0:
                continue
            noise = t.randn_like(p) * (scale * std)
            p.add_(noise)


def perturb_hidden_inplace(model: nn.Module, scale: float):
    if scale <= 0:
        return
    with t.no_grad():
        linear_layers = [m for m in model.net if isinstance(m, MultiLinear)]
...
if __name__ == "__main__":
    ...
    # Shared base init used for teacher and student before perturbation.
    ...
    for scale in PERTURB_SCALES:
        ...
        student_aux_hidden = MultiClassifier(N_MODELS, layer_sizes).to(DEVICE)
        student_all_hidden = MultiClassifier(N_MODELS, layer_sizes).to(DEVICE)
        student_aux.load_state_dict(shared_init.state_dict())
        student_all.load_state_dict(shared_init.state_dict())
        student_aux_hidden.load_state_dict(shared_init.state_dict())
        student_all_hidden.load_state_dict(shared_init.state_dict())

        perturb_model_inplace(student_aux, scale)
        perturb_model_inplace(student_all, scale)
        perturb_hidden_inplace(student_aux_hidden, scale)
        perturb_hidden_inplace(student_all_hidden, scale)
        ...
        distill(student_aux_hidden, teacher, GHOST_IDX, rand_imgs, EPOCHS_DISTILL)
        distill(student_all_hidden, teacher, ALL_IDX, rand_imgs, EPOCHS_DISTILL)

        acc_aux = accuracy(student_aux, test_x, test_y)
        acc_all = accuracy(student_all, test_x, test_y)
        acc_aux_hidden = accuracy(student_aux_hidden, test_x, test_y)
        acc_all_hidden = accuracy(student_all_hidden, test_x, test_y)
        ...
        records.append(
            {
                ...
                "student_aux_hidden_mean": float(np.mean(acc_aux_hidden)),
                "student_aux_hidden_ci95": ci_95(acc_aux_hidden),
                "student_all_hidden_mean": float(np.mean(acc_all_hidden)),
                "student_all_hidden_ci95": ci_95(acc_all_hidden),
            }
        )
    ...
    ax.errorbar(
        ...
        marker="o",
        label="Student (aux only, all params)",
        capsize=4,
    )
    ax.errorbar(
        df["perturb_scale"],
        ...
        marker="o",
        label="Student (all logits, all params)",
        capsize=4,
    )
    ax.errorbar(
        df["perturb_scale"],
        df["student_aux_hidden_mean"],
        yerr=df["student_aux_hidden_ci95"],
        marker="o",
        label="Student (aux only, hidden only)",
        capsize=4,
    )
    ax.errorbar(
        df["perturb_scale"],
        df["student_all_hidden_mean"],
        yerr=df["student_all_hidden_ci95"],
        marker="o",
        label="Student (all logits, hidden only)",
        capsize=4,
    )
    ax.set_xlabel("Perturbation scale (fraction of param std)")
    ax.set_ylabel("Test accuracy")
    ax.set_title("Effect of Student-Teacher Init Perturbation")
